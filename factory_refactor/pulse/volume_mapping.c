				 ...
				 enum ctl_dir ctl_dir)
{
	...
	int err;
	err = get_dB_range[ctl_dir](elem, &min, &max);
    mlog("set_normalized_volume 1,min:%ld max:%ld", min, max);
	if (err < 0 || min >= max) {
		err = get_raw_range[ctl_dir](elem, &min, &max);
        mlog("set_normalized_volume 5");
		if (err < 0)
			return err;
        mlog("set_normalized_volume 2");
    ...
    mlog("set_normalized_volume 4 val:%ld dir:%d", value, dir);
	return set_dB[ctl_dir](elem, channel, value, dir);
}
...
int set_normalized_capture_volume(snd_mixer_elem_t *elem,
				  ...
				  int dir)
{
    mlog("set_normalized_capture_volume");
