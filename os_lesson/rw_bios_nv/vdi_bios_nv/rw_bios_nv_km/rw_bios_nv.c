...
static long RW_BIOS_NV_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	GET_VAR_STRUCTURE *pGetVarStru=NULL;
	SET_VAR_STRUCTURE *pSetVarStru=NULL;
    ...
    if (err) 
        ...
        switch (cmd)
       {
	      ...
	      case RW_BIOS_NV_IOCGETBIOSNV:	
                  pGetVarStru = (GET_VAR_STRUCTURE *)kmallocmem;
		  retval= copy_from_user(pGetVarStru, (GET_VAR_STRUCTURE *)arg, sizeof(GET_VAR_STRUCTURE));
			if(retval!=0){
		  ...
		  printk("get var stru var_name:%x,var_value:%x,RetStatus:%x\n",pGetVarStru->var_name
			...
			if(RetStatus==1){
				retval = copy_to_user((GET_VAR_STRUCTURE *)arg,pGetVarStru,sizeof(GET_VAR_STRUCTURE));
			}
		  break;


		  case RW_BIOS_NV_IOCSETBIOSNV:
		  pSetVarStru =  (SET_VAR_STRUCTURE *)kmallocmem;
		  if (copy_from_user(pSetVarStru, (SET_VAR_STRUCTURE *)arg, sizeof(SET_VAR_STRUCTURE)))
          {
	      ...
	      //call smi set smbios struct
			...
			if(RetStatus==1){
				retval = copy_to_user((SET_VAR_STRUCTURE *)arg,pSetVarStru,sizeof(SET_VAR_STRUCTURE));
			}
