...
typedef void *(*NetThreadFunc)(void *);
...
{
public:
    NetThread(NetThreadFunc func, void *ctx, const char *name = NULL);
    ~NetThread() {};

    NetThreadStatus get_status() const;
    void set_status(NetThreadStatus status);
...
private:
    pthread_t       mThread;
    NetMutexCond    mMutexCond;

    NetThreadStatus mStatus;
    NetThreadFunc   mFunction;
    char            mName[THREAD_NAME_LEN];
    void            *mContext;

    NetThread();
    NetThread(const NetThread &);
    NetThread &operator=(const NetThread &);
};
