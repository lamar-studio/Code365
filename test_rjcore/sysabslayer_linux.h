...
int InitForEST(const char *comName, callback cb);
...
int getPowerState();
int setSleeptime(uint32_t second);
int checkIdleStatus(uint32_t second);
...
int getTerminalType(char *retbuf, size_t size);
...
int getDiskStatus_block(char *errcode, int len);
...
int getCurrentUsbHidInfo_block(char *info, int len);
int updateUsbHidBlacklist_block(const char* vid, const char* pid, int filter);
int checkUsbHidBlacklist(const char* vid, const char* pid);
